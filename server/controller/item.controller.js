...
class ItemController {
    async createItem(req, res) {
        ...
        if (userRole === 'admin') {
            ...
            let item
            if (id) {
                item = await db.query('UPDATE watch SET name = $1, description = $2, id_brand = $3, id_country = $4, quantity = $5, cost = $6 WHERE id = $7 RETURNING *', [
                    name,
                    description,
                    id_brand,
                    id_country,
                ...
                item = await db.query('INSERT INTO watch (name, description, id_brand, id_country, quantity, cost) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [
                    name,
                    description,
                    id_brand,
                    id_country,
